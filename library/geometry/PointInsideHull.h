 ...
 * (counter-clockwise order). The polygon must be such that every point on the
 * circumference is visible from the first point in the vector. It returns 0 for
 * points outside, 1 for points on the circumference, and 2 for points inside.
 * Usage:
...
int insideHull2(const vector<P> &H, int L, int R, const P &p) {
  ...
  int mid = L + len / 2;
  if (sideOf(H[0], H[mid], p) >= 0) return insideHull2(H, mid, R, p);
  return insideHull2(H, L, mid + 1, p);
}

int insideHull(const vector<P> &hull, const P &p) {
  if (sz(hull) < 3)
    return onSegment(hull[0], hull.back(), p);
  else
    return insideHull2(hull, 1, sz(hull), p);
}
