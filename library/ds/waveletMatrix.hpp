...
private:
  ...
  std::vector<int> pos;
  std::vector<std::vector<int64_t>> rui;

public:
  WaveletMatrix() {}
  WaveletMatrix(std::vector<int> vec) : WaveletMatrix(vec, *max_element(vec.begin(), vec.end()) + 1) {}
  // sigma:文字の種類数
  WaveletMatrix(std::vector<int> vec, const unsigned int sigma) { init(vec, sigma); }
  void init(vector<int> &vec, const unsigned int sigma) {
    ...
    for (unsigned int i = 0; i <= height; ++i) {
      rui[i].resize(static_cast<int>(A.size()) + 1);
      for (int j = 1; j <= static_cast<int>(A.size()); j++) {
        rui[i][j] = rui[i][j - 1] + A[j - 1];
      }
      if (i == height)
        break;
      std::stable_partition(A.begin(), A.end(), [&](int c) { return !get(c, height - i - 1); });
    }
  }
  ...
  int64_t topKsum(int k, int l, int r) {
    if (l == r) return 0LL;
    int64_t res = 0;
    int atai = 0;
    for (unsigned int i = 0; i < height; ++i) {
